...
def generate_launch_description():
    pkg = get_package_share_directory('omnibot_langchain')
    config_file = os.path.join(pkg, 'config', 'langchain_agent.yaml')

    use_sim_time = LaunchConfiguration('use_sim_time', default='false')
    vla_serve_url = LaunchConfiguration('vla_serve_url', default='http://localhost:8000')

    agent_node = Node(
        package='omnibot_langchain',
        ...
        parameters=[
            config_file,
            {
                ...
                # Pass API key from shell environment — never hard-code credentials
